...
class TmiInvite(WebSettingsDashboard):

    # hide unnecessary apps in settings/dashboard for specific user roles
    @http.route()
    def web_settings_dashboard_data(self, **kw):
        if not request.env.user.has_group('base.group_system') and not request.env.user.has_group('base.group_user'):
